...
int main(int argc, char *argv[])
{
	...
	char *outfile = (argc > i)? argv[i]:"flow.flo"; i++;
	...
	int   warps   = (argc > i)? atoi(argv[i]):PAR_DEFAULT_NWARPS;  i++;
	float TOL     = (argc > i)? atof(argv[i]):PAR_DEFAULT_TOL;     i++;
	int   maxiter = (argc > i)? atoi(argv[i]):PAR_DEFAULT_MAXITER; i++;
	...
	if(correct1 && correct2 && nx == nx1 && ny == ny1)
	{
		...
		horn_schunck_pyramidal(
				I1, I2, u, v, nx, ny,
				alpha, nscales, fscale, zfactor, warps, TOL, maxiter,
				verbose
				);
