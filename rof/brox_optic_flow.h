...
)
{
	...
	du[k] =
		(1. - w) * du[k] + w * (Au[k] - D[k] * dv[k] + alpha * div_du) / Du[k];
	dv[k] =
		(1. - w) * dv[k] + w * (Av[k] - D[k] * du[k] + alpha * div_dv) / Dv[k];
...
)
{
	...
	for (int no = 0; no < outer_iter; no++)
	{
		...
		for (int ni = 0; ni < inner_iter; ni++)
		{
			...
			psi_gradient (I1x, I1y, I2wx, I2wy, I2wxx, I2wxy, I2wyy, du, dv,
					psig, nx, ny);
			...
			for (int i = 0; i < size; i++)
			{
				...
				const float GDu =
					g * (I2wxx[i] * I2wxx[i] + I2wxy[i] * I2wxy[i]);
				const float GDv =
					g * (I2wyy[i] * I2wyy[i] + I2wxy[i] * I2wxy[i]);
			...
			while (error > TOL && nsor < MAXITER)
			{
				...
				for (int i = 1; i < ny - 1; i++)
				{
					for (int j = 1; j < nx - 1; j++)
						...
						error +=
							sor_iteration (Au, Av, Du, Dv, D, du, dv, alpha, psi1,
									psi2, psi3, psi4, i, nx, nx, j, nx, 1,
									1);
				}
				...
				for (int j = 1; j < nx - 1; j++)
				{
					error += sor_iteration (Au, Av, Du, Dv, D, du, dv, alpha,
							psi1, psi2, psi3, psi4,
							0, 0, nx, j, nx, 1, 1);
